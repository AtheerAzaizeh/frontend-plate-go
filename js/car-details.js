document.addEventListener("DOMContentLoaded", () => {
  ...
  if ((!plate && !reportId) || !token) {
    return showModalMessage("Invalid request.");
  }
  ...
  if (reportId && !plate) {
    fetch(`${BACKEND_URL}/api/reports/${reportId}`, {
      ...
      .catch((err) => {
        ...
        showModalMessage("Report not found");
      });
  } else {
  ...
  function fetchCarDetails(plateNumber) {
    fetch(`${BACKEND_URL}/api/reports/car/${plateNumber}`, {
      ...
      .catch((err) => {
        ...
        showModalMessage("Car not found");
      });
  }
  ...
  reportBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      showModalMessage("Failed to send report");
    }
  });
  ...
  document.getElementById("send-message").onclick = async () => {
    const currentPlate = carPlate || plate;
    if (!currentPlate) return showModalMessage("Plate not found");
    ...
    try {
      ...
      showModalMessage("Failed to start chat: " + err.message);
    }
  };
